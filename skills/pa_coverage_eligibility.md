...
Get procedure codes from Input **procedures_requested** (use the code field for each procedure) and pass each to `lookup_cms_coverage(cpt_code=...)`. For devices or drugs mentioned in the case, pass the device or drug name to `lookup_fda(name=...)`. Do not call tools without these values from the Input.
